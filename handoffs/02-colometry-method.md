...
**The fronting paradox — marked word order argues for MERGE, not split.** When a grammatically tight unit (verb + its required-case complement, or other bound constituent) appears in a *marked* word order — e.g., object fronted before its governing verb, or before a subordinator introducing the verb — the natural editorial instinct is to split at the fronted element as a way of "visualizing the emphasis." **This instinct is wrong.** The rhetorical effect of fronting depends on the grammatical unity *staying intact*. The marked arrangement is felt as emphatic precisely because the hearer processes the fronted element in a non-default position *within a single breath unit*, against the rhythm of the expected order. Splitting at the fronted-element boundary mechanizes the emphasis — it imposes a pause that was not in the original oral delivery — and paradoxically *diminishes* the rhetorical force by converting a felt rhetorical move into a marked editorial one. **The fronted arrangement IS the atomic thought, complete within its marked order. Breaking it converts rhetoric into cartography.**

Verb + its required-case object (e.g., μνημονεύω + genitive, ἀκούω + genitive for the person heard, ἄρχω + genitive, δέομαι + genitive, γεύομαι + genitive) is the canonical case where this applies — even when the object has been fronted for rhetorical emphasis, the grammatical unity should be preserved on one line. The reader can hear the fronting from the word order alone; mechanizing it with a line break removes the marked-against-default quality that made the rhetoric work.

*Case studies:*
- **Gal 2:9 — split** (pillars characterization on own line): two distinct thoughts (the named persons vs. their ironic characterization). Subject + substantival-participial-phrase appositive, where the appositive is six words, non-trivial, and introduces the δοκοῦντες motif. Grammatical warrant for split is the substantival-participle-as-own-thought rule, not the rhetorical motif itself.
- **Gal 2:10 — merge** (fronted τῶν πτωχῶν kept with μνημονεύωμεν on one line): μνημονεύω requires genitive for its object. The fronted genitive + restrictive μόνον create a marked word order whose rhetorical force depends on the grammatical unity staying intact. Splitting would replace felt emphasis with marked emphasis.

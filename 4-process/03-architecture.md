...
**Usage:**
```bash
PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/auto_colometry.py              # all 27 books
PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/auto_colometry.py --book Mark  # one book
PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/auto_colometry.py --book Mark --chapter 4  # one chapter
```
...
**Usage:**
```bash
PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/v2_colometry.py                    # all 27 books
PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/v2_colometry.py --book Acts         # one book
PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/v2_colometry.py --book Acts --chapter 1  # one chapter
```
...
**Usage:**
```bash
PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/ylt_parse.py
```
...
**Usage:**
```bash
PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/ylt_align.py                    # all 27 books
PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/ylt_align.py --book Acts         # one book
PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/ylt_align.py --book Acts --chapter 9  # one chapter
```
...
**Usage:**
```bash
PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/build_books.py               # all books
PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/build_books.py --book mark    # one book
```
...
> **Superseded 2026-05-12.** The paragraph above describes the pre-migration v4/eng-kjv pipeline (structural English glosses, retired Wave 6). The current cascade is **Greek edit → `regenerate_english.py` (KJV redistribution via Strong's-matching) → `build_books.py` (rebuild with `.swap` injection) → commit.** The v4/eng-kjv layer is now KJV verbatim distributed per Greek ATU line by `atu_method.kjv_alignment.align_verse()`; when Greek lines change, KJV is redistributed (not rewritten). The cascade rule still holds — but the middle step has different semantics.
...
PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/build_books.py
# Then commit the changed books/*.html files
```

If re-running the auto-formatter (caution — overwrites hand edits):
```bash
PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/auto_colometry.py
PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/build_books.py
```
...
**`5-machinery/scripts/apply_vocative_merges.py`** — applies the APPOSITION-CANDIDATE merges. Supports `--english` (to apply to v4/eng-kjv), `--save-candidates` / `--load-candidates` (to share the candidate list between Greek and English passes without re-scanning), and `--dry-run`. Processes merges within a verse in reverse order so that deletions don't shift earlier line indices.

**`5-machinery/scripts/scan_no_anchor_lines.py`** — finds every line in the corpus that lacks a thought-marking anchor (finite verb, infinitive, participle, or head substantive in N/A/D/V case). Features:
...
**`5-machinery/scripts/verify_word_order.py`** recognizes these markers. The loader splits a line at each superscript digit and assigns the post-marker content to the indicated verse for word-order comparison. This preserves the SBLGNT integrity guard: the per-verse word list still matches the source even when the colometric file has merged visual lines across verse boundaries.

**`5-machinery/scripts/build_books.py`** renders inline superscripts as `<sup class="verse-marker" id="v-{chapter}-{verse}-inline">N</sup>` HTML elements. The verse-marker is visible inline in the flowing colometric text; the anchor id lets TOC/search functionality jump to the exact inline location where the subsequent verse begins. Verse `div` blocks still carry their primary `id="v-{chapter}-{verse}"` anchors so citation targeting works unchanged.
...
PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/apply_no_anchor_merges.py --save-candidates /tmp/merges.json

# 2. Apply to Greek
PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/apply_no_anchor_merges.py --load-candidates /tmp/merges.json

# 3. Apply same candidates to English
PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/apply_no_anchor_merges.py --load-candidates /tmp/merges.json --english

# 4. Rebuild
PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/build_books.py

# 5. Re-scan to confirm residual
PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/scan_no_anchor_lines.py --summary-only
```
...
v0 is new in this restructure. It is a per-chapter split of `sblgnt-source/*.txt` into `v0-prose/{NN-book}/{abbrev}-{NN}.txt`, with the SBLGNT apparatus markers (`⸀ ⸁ ⸂ ⸃ ⸄ ⸅`) **retained**. Keeping the markers is a deliberate choice: v0 should be a faithful slice of the published scholarly text, not a preprocessed derivative. The first apparatus-marker stripping happens at the v1 step as part of `5-machinery/scripts/auto_colometry.py`, where it has always happened. A reader diffing `sblgnt-source/Matt.txt` against `v0-prose/01-matt/matt-*.txt` should see only chapter splitting, no content changes.

v0 is generated by `5-machinery/scripts/build_v0_prose.py`, which reads each file in `sblgnt-source/`, splits on the `{BookAbbrev} {chapter}:{verse}` line prefix, and writes per-chapter files into the book-subfolder layout.
...
- `5-machinery/scripts/v3_colometry.py` — writes to `v3-colometric/{NN-book}/{abbrev}-{NN}.txt`
- `5-machinery/scripts/build_books.py` — the v3 fallback in `resolve_greek_path` now walks the book-subfolder layout to find v3 files. In practice the fallback is dead code (every chapter has v4), but it's kept correct for safety.

### Script added

- `5-machinery/scripts/build_v0_prose.py` — NEW. Chapter-split tool that generates `v0-prose/` from `sblgnt-source/`. See the top-level `data/text-files/README.md` for invocation.
...
- **Acts 2:38 italic-attachment.** Source-data issue: v4/grk lacks φησὶν but TAGNT has it; the extractor's surface-lookahead absorbs the orphan TAGNT token into cola 3. Fix in token-to-cola assignment logic in `5-machinery/scripts/regenerate_english.py`.
...
- `3-project/00-index.md` — stale `02-colometry-method.md` and `YLT-visualization.md` rows deleted; atu-method pointer added
- `3-project/01-overview.md` — 2026-05-12 KJV migration block appended
- `3-project/02-text-editorial.md` — universal-layer pointer block at top
...
- `5-machinery/scripts/build_books.py:34` — `regenerate_english_kjv.py` → `regenerate_english.py` in comment

**Pending — separate commit.** Restructure of `private/01-method/colometry-canon.md` to mirror the bofm v3.0 framework-pointer-only pattern: seven §-subsections become pointer-only cross-references to `atu-method/1-method/framework.md`, with GNT-specific rule body preserved intact. That work requires a §6.5 audit dispatch (named-category + cross-project triggers) and lives in its own commit with explicit audit-evidence.

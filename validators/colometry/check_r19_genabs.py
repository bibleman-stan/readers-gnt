...
def _gen_abs_candidates(tokens: list) -> list[tuple]:
    """Return [(ptc_word, ptc_lemma, subj_word, subj_lemma)] for likely gen abs on this line.
...
def check_book_chapter(book: str, chapter: int) -> List[Candidate]:
    ...
    for verse in verses:
        ...
        for line in verse["lines"]:
            ...
            if gen_abs:
                fins = [(w, lm) for w, pos, p, lm in lt if is_finite_verb(pos, p)]
                if fins:
                    ptc, ptc_l, subj, subj_l = gen_abs[0]
                    fin, fin_l = fins[0]

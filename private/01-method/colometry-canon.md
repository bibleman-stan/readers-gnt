...
- **Bidirectional atomic-thought test (codified at framework §1.1, 2026-05-13).** "Single cognitive bite" requires the line to stand on its own **referentially**, not just **grammatically**. A line whose content is anaphoric to prior context fails atomic-thought standing alone, even when forward grammatical closure passes. The test is asymmetric: **anaphoric** unresolved-backward-dependence FAILS; **cataphoric** forward-pointing reference PASSES. The reader can process a line whose content is whole and merely sets up what follows (cataphoric); the reader cannot process a line whose content is a pointer to undefined prior context (anaphoric). See [`atu-method/docs/framework.md §1.1`](../../atu-method/docs/framework.md) for full statement.
  ...
  **GNT diagnostic markers for anaphoric failure.** When a line's content is dominated by these markers AND the line stands alone (no apodosis on the same line), the bidirectional test fires and the line MUST merge with its apodosis:
  ...
  - **Anaphoric-genitive-absolute formulae**: `Ταῦτα δὲ αὐτοῦ λέγοντος/εἰπόντος/ἀκούσαντος` (while/after he was saying/saying-having-said/hearing these things — gen abs containing `ταῦτα` referencing prior speech)
  ...
  **Relationship to GNT canon rules.** This is a §1.1 test-refinement, NOT a new §1.5 merge-override. R19 (gen abs always own line) still applies when the gen abs is referentially self-contained (`Τοῦ δὲ Ἰησοῦ γεννηθέντος ἐν Βηθλέεμ…` — Matt 2:1, the subject Ἰησοῦ is a proper noun, not anaphoric; gen abs splits per R19). M4-GNT-1 (subject-orphan) still applies when the orphan-subject is referentially self-contained. The bidirectional test catches a class of cases UPSTREAM of these mechanisms — lines whose content is dominated by anaphoric markers fail atomic-thought standing alone and merge with the apodosis regardless of which §1.5 mechanism would otherwise apply. Per framework §1.2 constraint-vs-determination asymmetry: grammar can constrain ATU boundaries; atomic-thought (now stated bidirectionally) determines them.
  ...
  **Worked GNT cases:**
  ...
  - **Acts 1:9 anaphoric gen abs** — `καὶ ταῦτα εἰπών` contains `ταῦτα` referencing prior speech upstream. Per bidirectional test, this line standing alone fails (the reader has no on-line content for "these things"). The current canon already MERGES this with the apodosis (`καὶ ταῦτα εἰπών / βλεπόντων αὐτῶν / ἐπήρθη…` — three lines in the showcase, but `καὶ ταῦτα εἰπών` is the FEF opener that the apodosis `ἐπήρθη` resolves; the bidirectional test confirms this merge intuition). Backward-anaphoric `ταῦτα` is what makes `καὶ ταῦτα εἰπών` NOT a standalone ATU.

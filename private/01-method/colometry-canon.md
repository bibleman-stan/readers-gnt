...
| **R17 developmental connectives** (clause does NOT bind backward) | δέ, γάρ, μέν *(excludes καί/τε/γε/δή — continuity/emphasis, not development; οὖν held — Johannine resumptive)* | §3.8 | `scripts/sblgnt_generate.py` (`_DEV_CONNECTIVE` / `_dev_connective`) |
...
| **R9 subordinate bind / R25 ὥστε** | no conflict — R9 does NOT touch ὥστε; consecutive-result ὥστε+inf is R25's exclusive domain, illative ὥστε+finite stays split outside R9 | ὥστε is a §3.4 carve-out (#1) | §3.14a; §3.4 carve-out 1 |
...
2. **Cross-verse (finite vs. non-finite — the within-verse window gates *predications*).** The framework §3 within-verse binding constraint gates **second predications**, not sub-predicational parts. So: a **finite** subordinate/relative clause whose governor is in another verse → **REVIEW** (stays split) — a cross-verse *finite chain* is multiple predications (Acts 1:1-3 `ὧν…ἄχρι ἧς…ἀνελήμφθη ³οἷς…παρέστησεν` collapsed three predications into one mega-line). But a **non-finite** dependent (participle / infinitive) is one predicate's part and **rides its finite governor even across a verse marker** — this preserves the Greek-prominent cross-verse cases (speech frames `²λέγοντες` / `λέγων`; supplementary participles `οὐ παύομαι … ¹⁶εὐχαριστῶν`, Eph 1:15-16; circumstantial-participle chains, Mark 5:25-27) that a blanket verse-split wrongly severs. *(This finite/non-finite distinction is the Greek-specific elaboration of framework §3's within-verse rule, which was Hebrew-validated; added 2026-05-24 after the §7.3 audit pair flagged the non-finite over-split. Implemented in `is_root()` — finite relative/cognition-ὅτι gate on same-verse governor; participle/infinitive bind to any-verse governor.)* Residual: a backward subordinator/preposition can strand on the prior verse's line when the verse boundary bisects its finite clause (e.g. `…διδάσκειν ²ἄχρι |`) — a v2/editorial residual class.
...
**Implementation (mechanical-first).** R9 is a **v1.5 binding rule**, encoded in `scripts/sblgnt_generate.py`. Two cooperating stages: (1) `generate()`/`is_root()` does the nesting-based ATU assignment off the lowfat clause structure, with the within-verse guard of carve-out 2 (finite relative/cognition-ὅτι gate on a same-verse governor; non-finite participle/infinitive ride any-verse governor); (2) `merge_subordinate_clauses` applies the forward/backward/comparative subordinator binding, each gated by `_bind_ok` = within-verse + ≤2 finite verbs + content cap, with the coordinate-relative guard in `_rel_is_correlative` and quote protection via the speech-frame `qflag`; (3) **line-end leader integrity** (`merge_line_end_leaders` + `lead_forward_trailing_governors`) enforces that no line *ends* on a forward-governing leader. The corpus is generated v0→v1→v1.5; this revision propagates by regeneration. The legacy split-detector `validators/syntax/check_r9_subordinate_clause.py` is **non-driving** (it enforced the retired own-line default); do not invert it to re-merge.

**Forward-governing leader integrity (placement rule — orthogonal to the boundary rules).** A *forward-governing leader* is a function word that governs what FOLLOWS it: an **article** (R3), a **framing device / coordinator** (R8: ἰδού/διό/οὖν/ἀλλά/γάρ/πλήν/τοιγαροῦν, καί), a **subordinator** (R9: the forward-frame / backward-completing / comparative sets), or a **preposition** (ἐν/εἰς/ἐκ/πρός/διά/κατά/… and improper prepositions ἄχρι/ἕως/μέχρι/πρίν). **No ATU line may END on one** — "ending a thought on *until* is incoherent"; the leader must LEAD the unit it introduces. This is a single closed-list principle (the four buckets above), not a per-token patch, and it is a *placement/rendering* rule — it decides which side of an already-decided boundary a leader token renders on; it does NOT decide where boundaries fall (contrast §3.4 binding and the §2 marker-license). Two mechanisms: a line that is *only* a leader STRONG-MERGES forward into the next (`merge_line_end_leaders`); a content line with a *trailing* governor (stranded by a verse boundary or a parse attachment — Acts 1:1 `…διδάσκειν ἄχρι` | `1:2 ἧς ἡμέρας…`) RELOCATES just the trailing governor to lead the next line, the rest of the line intact (`lead_forward_trailing_governors`, 2026-05-24). Strand count after the rule: 0 corpus-wide (was 3: Acts 1:1, Acts 7:18, 1Pet 3:3).
...
**v1.5 binding-stage operationalization (2026-05-24).** R17's δέ-pivot diagnostic generalizes into the binding engine as a **backward-bind blocker**: a clause headed by a developmental connective (`_DEV_CONNECTIVE = {δέ, γάρ, μέν}` — δέ +development, γάρ +grounds, μέν correlative-setup) does NOT bind backward into the prior line, because it heads its own development/grounds beat (`scripts/sblgnt_generate.py::_dev_connective`, consumed in `merge_subordinate_clauses`' backward + comparative passes). This fixed Rom 2:1 (`ἐν ᾧ γὰρ κρίνεις … κατακρίνεις` and `τὰ γὰρ αὐτὰ πράσσεις` were over-merged with the prior vocative). It is the same claim as R8 (framing devices lead their content) and R14 (men/de members stack as separate ATUs) — viewed from the bind direction. **Firewall-safe**: these are clause-level connectives heading their own finite predication, so the rule confirms the primary bidirectional test (it never licenses a sub-predication split — contrast the §2 marker-license for sub-clausal asseveratives like *yea*). Closed list excludes καί/τε (continuity/tight-coordination → bind) and γε/δή (emphatics); **οὖν held** (inferential in Paul resists, but Johannine οὖν is resumptive ≈ continuity — and the bind-back gate already neutralizes it: only 1 of ~198 Johannine οὖν even reaches the rule). High-precision, low-recall: presence signals a boundary, absence implies nothing (asyndeton breaks too). §7.3 audit pair (Greek-discourse + consistency, 2026-05-24): SOUND / COHERENT — zero +6pp regression (7/7 measured chapters byte-identical), no R8/R14 conflict.
...
**Status:** Active
**Category:** A (closed-list-eligible subject shapes — C1–C5) / B (length-backstop / multi-line restructuring)
**Decidability:** UD-pattern
**Layer:** 3
...
**Scope.** A v4/grk line whose content is a subject NP of one of the closed-list-eligible shapes, with the matrix predicate orphaned on the immediately-next v4/grk line. M4-GNT-1 is the GNT-specific Tier 4 merge-override operationalization of framework M4.

**Exclusions** (closed list — each cites dominating rule).
1. **G1: Attributive participle on line B** (modifier of the NP on line A, not the main-clause verb). When line B begins with a participial form that is attributive to the NP on line A, the participle is a modifier continuation, not a bare finite predicate. STAY-SPLIT. → R20
2. **G2: Verbless nominal-sentence line A** (predicate NP or predicate ADJ; complete by Greek ellipsis). When line A is itself a complete verbless clause (implied `ἐστίν`), the following line is a separate predication. STAY-SPLIT. → §1.2 complement integrity
3. **G3: Periphrastic-construction line B** (ἦν/ἐστίν/ἔσται + participle forming a single periphrastic predicate). STAY-SPLIT. → R5
4. **G4: Doxological vocative-NP + infinitival-complement** (pattern: `Ἄξιος εἶ` + vocative NP + `λαβεῖν` infinitive). The infinitive is a complement of a prior predicate. STAY-SPLIT. → R10
5. **G5: Line A has only participial verbs (no finite indicative/subjunctive)** → eligible for merge *despite* the surface `a_verb` heuristic firing on line A. M4-GNT-1 FIRES.
6. **R7 vocative-only on line A.** Bare vocative address; not a grammatical subject. → R7
7. **R11 / J3 speech-act parentheticals.** Line A ends in speech-intro tag (e.g., `λέγει·`). → R11
...
- *Compliant (C1 household-code):* `Αἱ γυναῖκες, ὑποτάσσεσθε τοῖς ἀνδράσιν, ὡς ἀνῆκεν ἐν κυρίῳ.` (Col 3:18 after merge)
...
- *Non-compliant:* `Αἱ γυναῖκες,` / `ὑποτάσσεσθε τοῖς ἀνδράσιν, ὡς ἀνῆκεν ἐν κυρίῳ.` — subject NP standing alone fails the No-Anchor test (no predication on the line); bare predicate on line B has no anchor. Must be merged per the rule.
- *Excluded by G1:* `Δημήτριος γάρ τις ὀνόματι, ἀργυροκόπος,` / `ποιῶν ναοὺς ἀργυροῦς Ἀρτέμιδος παρείχετο…` — line B `ποιῶν` is attributive participle of `ἀργυροκόπος`. STAY-SPLIT. → R20 (Acts 19:24)
...
# (Full signature inline at §3.18; this block is the rule-ID summary.)
M4-GNT-1:
  rule_id: M4-GNT-1
  category: A / B (multi-valued — see §3.18 Category field)
  layer: 3
  precedence_tier: 4
  signature_location: canon §3.18 (full UD-trigger + G1–G5 + universal-6 exclusions)
  scholarship: private/01-method/scholarship/m4-gnt-1.md
~~~
...
**Follow-on — cross-verse over-merge fix (2026-05-24, after Acts 1:1-3 surfaced it):** Acts 1:1-3 deployed as one cross-verse mega-line — `is_root()` bound finite restrictive-relative chains to their governor with no within-verse guard (the gate existed only in `merge_subordinate_clauses`, not the fabric). Fix = the carve-out 2 within-verse guard in `is_root()`. A §7.3 audit pair on the first (blanket same-verse) form flagged that it over-split **non-finite** cross-verse binds (speech frames, supplementary participles Eph 1:15-16, circumstantial chains Mark 5:25-27); refined to **finite-only** gating (the predication-level reading above), which preserves those. Net delta ~+12 lines across the 14 measured chapters, all genuine finite-predication cross-verse splits; within-verse +6pp binds byte-identical (Mark 4 unchanged). Audit reasoning: `C:\tmp\audit_withinverse_{oversplit,regression}.md`.

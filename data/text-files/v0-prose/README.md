...
**Apparatus markers retained.** The SBLGNT's manuscript-variant markers (`⸀ ⸁ ⸂ ⸃ ⸄ ⸅`) are preserved as-is. v0 is a faithful slice of the published scholarly text; the first stripping happens at the v1 step, deterministically, as part of `scripts/auto_colometry.py`.
...
- **Downstream of v0:** `v1-colometric/` is produced from `sblgnt-source/` (not from v0 directly, though the content is equivalent) via `scripts/auto_colometry.py`. v0 exists so that readers/reproducers have a per-chapter view of the starting text that visually matches every downstream tier's layout.

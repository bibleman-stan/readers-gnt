...
Discovers all validator modules in 5-machinery/validators/syntax/ and 5-machinery/validators/colometry/,
...
Modes:
    ...
    --baseline-check      Compare current per-rule counts to 5-machinery/validators/.baseline.json.
                          Exit 1 if any rule's count INCREASED vs baseline.
                          Used by 5-machinery/validators/hooks/pre-commit.
    --update-baseline     Capture current per-rule counts as the new baseline at
                          5-machinery/validators/.baseline.json. Used after intentional changes.
...
Usage:
    ...
    PYTHONIOENCODING=utf-8 py -3 5-machinery/validators/run_all.py --output report.md
"""
...
def discover_validators():
    """Import all check_* modules from 5-machinery/validators/syntax/ and 5-machinery/validators/colometry/.
...
def collect_candidates(books_filter=None, validators_filter=None, verbose=False):
    ...
    if not validators:
        print("No validator modules found in 5-machinery/validators/syntax/ or 5-machinery/validators/colometry/.",
              file=sys.stderr)
...
def main(output_path=None, books_filter=None, validators_filter=None, verbose=False,
    ...
    if baseline_check:
        ...
        if baseline is None:
            print(
                "  No baseline at 5-machinery/validators/.baseline.json. Run with --update-baseline\n"
        ...
        if regressions:
            ...
            print("    PYTHONIOENCODING=utf-8 py -3 5-machinery/validators/run_all.py --update-baseline",
                  file=sys.stderr)
            return 1
...
def _build_parser():
    ...
    p.add_argument(
        "--baseline-check",
        action="store_true",
        help="Compare current counts to 5-machinery/validators/.baseline.json; exit 1 on regression.",
    )
    p.add_argument(
        "--update-baseline",
        action="store_true",

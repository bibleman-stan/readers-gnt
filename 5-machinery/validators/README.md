...
PYTHONIOENCODING=utf-8 py -3 5-machinery/validators/run_all.py --summary
...
PYTHONIOENCODING=utf-8 py -3 5-machinery/validators/run_all.py --output reports/validator-run.md

# Restrict to specific books or rules
PYTHONIOENCODING=utf-8 py -3 5-machinery/validators/run_all.py --summary --books matt mark
PYTHONIOENCODING=utf-8 py -3 5-machinery/validators/run_all.py --summary --validators R2 R18
```
...
Calls `5-machinery/validators/check_canon_extensions.py` with the proposed commit
...
```bash
cp 5-machinery/validators/hooks/pre-commit .git/hooks/pre-commit
cp 5-machinery/validators/hooks/commit-msg .git/hooks/commit-msg
...
`5-machinery/validators/colometry/check_m4_gnt_1_subject_orphan.py` is the reference for tier-yield discipline. It documents 5 G-exclusions + 6 universal exclusions, each explicitly commenting which higher-priority rule governs at the yield point:
...
When implementing a new detector at Layer 3 (`5-machinery/validators/colometry/`), follow this pattern: list every higher-priority rule the new detector might overlap with, add an exclusion check for each, comment which rule governs at the yield point.
...
- **`5-machinery/validators/run_all.py`** — alphabetical iteration; no runner-level precedence. With ~13 implemented detectors and the per-detector yield pattern above, runner-level ordering is not critical yet. Revisit when detector count grows or when a future detector cannot enforce its own yields internally.
...
- **Lemma-bag straddles MetaV record-keys (50 v4/grk lines; `data/eng-kjv-coverage.baseline.json`).** TAGNT and MetaV are storage-indexed by Stephanus 1551 verse-keys — purely an addressing convention per canon §1 (versification is editorial overlay, not an analytical signal). The `align_verse(book, ch, vs, ...)` API currently restricts its KJV pool to `MetaV[(book, ch, vs)]`. When v4/grk's editorial ATU-segmentation puts Greek content on a line whose Strong's-lemma signature is matched by KJV words stored under an *adjacent* MetaV record, the distributor can't reach the matching English and the v4/grk line gets blank output. Canonical instances: 50 lines indexed in the baseline (`acts-05.txt 5:39 line 2 ἐπείσθησαν`, `eph-05.txt 5:14 line 0 πᾶν γὰρ τὸ φανερούμενον`, `matt-14.txt 14:27 line 3 μὴ φοβεῖσθε`, etc.). The MetaV-record offset is a substrate-storage artifact; the underlying ATU is intact in v4/grk.

  **Design sketch (record-neighbor-pool extension).** Three components:

  1. **Neighbor-record candidate index.** Pre-compute per v4/grk line whether its source lemmas have ZERO Strong's matches in its host MetaV record but ≥1 match in an adjacent record. Output: `{(book_slug, line_id) → (neighbor_record_keys, confidence)}` table cached at `data/neighbor-record-candidates.json`. The MetaV record-key tuple is named explicitly as a substrate-addressing artifact, not an analytical category.

  2. **align_verse extension.** Take an optional `neighbor_record_pool: dict[record_key → list[KjvWord]]` parameter. When a source line is in the candidate index, allow Pass A1 / Pass B / Pass C to draw from neighbor records. Mark pooled-claim words in output for downstream verification (`provenance` field indicating "drawn from neighbor MetaV-record vpos 3").

  3. **regenerate_english.py wiring.** When processing a chapter, load MetaV for the full chapter once and pass each call a neighbor pool spanning the surrounding records. Cross-chapter substrate-record boundaries handled by chapter-pair load.

  **Engineering cost estimate.** Substantial — multi-session work. Three reasons: (a) `align_verse`'s internal logic assumes per-record claim accounting and would need to track pool-spanning claims to avoid double-claiming; (b) the candidate index needs corpus-wide calibration (50 visible-failures is the lower bound; true-positive pool-spanning lines could be 200+, false-positives would silently move correct KJV to wrong lines); (c) needs cross-corpus consideration since `atu-method/atu_method/kjv_alignment/` is shared infrastructure — bofm/tanakh inherit.

  **Recommended sequence** (when prioritized):
  1. Investigation pass: scan corpus-wide for pool-spanning candidates beyond the 50 baseline lines; quantify TP/FP rate.
  2. Adversarial audit dispatch (≥2 parallel) on the proposed `align_verse` extension before any code.
  3. Implement at `atu-method/atu_method/kjv_alignment/record_neighbor_pool.py` (new module) — preserves the single-record `align_verse` as an importable API for callers that don't want pooling.
  4. `regenerate_english.py` opt-in via flag (`--neighbor-pool`) initially; promote to default after corpus verification.
  5. Three-check verification + baseline update.

  **Cross-corpus consideration.** Tanakh likely has analogous MetaV-record-key offsets (different storage indexing between BHS-record and KJV-OT-record for the same ATU content). bofm has no analog (single English source). Design the extension as corpus-agnostic — caller passes `neighbor_record_pool`, library handles distribution. **Critical naming discipline**: do NOT call this "cross-verse alignment" or "versification mismatch" — those framings concede that verse-keys are analytical units. They are substrate-addressing artifacts. The new module name is `record_neighbor_pool.py`, the flag is `--neighbor-pool`, the index is "neighbor-record candidates" — not "cross-verse spillover."

...
Read-only accessors over Macula Greek constituent data. Used by detectors under `5-machinery/validators/` and by ad-hoc analysis.
...
The dedicated shared modules `5-machinery/validators/_shared/macula_clauses.py` and `5-machinery/validators/_shared/morphgnt_lookup.py` are the preferred entry points for detector code; the helpers here are for one-off Macula exploration.
...
Completed-sweep appliers move to `archive/` once the corpus is clean. The detector home for newly-codified canon rules is `5-machinery/validators/syntax/*` (Layer-1 break-legality, mechanical) and `5-machinery/validators/colometry/*` (higher-layer signature checks).
...
Non-routine investigations kept in `5-machinery/scripts/` because they remain referenced by ongoing canon work.
...
- Scripts that wrap canon-named detectors emit `5-machinery/validators/_shared/` `Candidate` objects via `emit_candidate` (post verdict-layer migration, 2026-05-16).

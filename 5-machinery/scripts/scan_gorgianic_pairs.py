...
Scans data/text-files/v1.5/grk/ for gorgianic pair candidates:
...
Usage:
  PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/scan_gorgianic_pairs.py
"""
...
def scan_file(filepath: Path) -> List[Dict]:
    """Scan one v1.5/grk file and return list of candidate dicts."""
    candidates = []
    try:
...
def scan_corpus() -> List[Dict]:
    """Walk all v1.5/grk files and collect candidates."""
    all_candidates = []
...
def write_report(candidates: List[Dict]) -> None:
    ...
    lines_out.append("")
    lines_out.append("**Scanner:** `5-machinery/scripts/scan_gorgianic_pairs.py`")

#!/usr/bin/env python3
"""
Automatic mechanical fixer for v1.5/grk colometric files.
...
Usage:
    PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/v4_auto_fix.py          # apply fixes
    PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/v4_auto_fix.py --dry-run # report only
"""
...
# Map from v1.5/grk file prefix to Macula XML filename
PREFIX_TO_XML = {
    'matt':    '01-matthew.xml',
    'mark':    '02-mark.xml',
    'luke':    '03-luke.xml',
...
# Map from v1.5/grk file prefix to MorphGNT filename
PREFIX_TO_MORPHGNT = {
...
def parse_v4_file(filepath):
    """
    Parse a v1.5/grk file into a list of verse blocks.
...
def process_file(filepath, vocatives, chapter_num, morphgnt_data=None, dry_run=False):
    """Process a single v1.5/grk file. Returns list of fix records."""
...
def main():
    parser = argparse.ArgumentParser(description='Auto-fix mechanical colometric violations in v1.5/grk files')
    parser.add_argument('--dry-run', action='store_true', help='Report only, do not write changes')
    args = parser.parse_args()

    if not os.path.isdir(V4_DIR):
        print(f"ERROR: v1.5/grk directory not found: {V4_DIR}", file=sys.stderr)
        sys.exit(1)

    # Gather all v1.5/grk files from book subfolders
    files = []
    for book_dir in sorted(os.listdir(V4_DIR)):
        ...
        for fname in sorted(os.listdir(book_path)):
            if fname.endswith('.txt'):
                files.append(fname)
    print(f"Found {len(files)} v1.5/grk files")
    if args.dry_run:
        print("DRY RUN — no files will be modified\n")
    else:
        print("LIVE RUN — files will be modified\n")

    # Pre-load vocatives for all books that have v1.5/grk files
    book_vocatives = {}
    ...
    # Pre-load MorphGNT data for all books that have v1.5/grk files
    book_morphgnt = {}

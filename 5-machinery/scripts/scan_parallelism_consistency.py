...
Walks data/text-files/v1.5/grk/ looking for spans where N >= 3 parallel
...
Usage:
  py -3 5-machinery/scripts/scan_parallelism_consistency.py
"""
...
V4_DIR = REPO_ROOT / "data" / "text-files" / "v1.5" / "grk"
...
def parse_file(filepath: Path) -> List[dict]:
    """
    Parse a v1.5/grk chapter file into a list of line records.
...
def scan_corpus() -> List[dict]:
    """Walk all v1.5/grk chapters and collect drift candidates."""
...
def write_report(candidates: List[dict], output_path: Path) -> None:
    ...
    lines = [
        ...
        "**Corpus:** data/text-files/v1.5/grk/ — all 260 chapters",
        "",
        "---",
        "",
        "## Summary",

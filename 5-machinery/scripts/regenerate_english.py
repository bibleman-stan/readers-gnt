...
Usage
-----
  python 5-machinery/scripts/regenerate_english.py --book matt
  python 5-machinery/scripts/regenerate_english.py --all
  python 5-machinery/scripts/regenerate_english.py --book matt --force
"""
...
REPO_ROOT    = Path(__file__).resolve().parent.parent.parent       # readers-gnt/
...
def normalise_greek(s: str) -> str:
    ...
    Strong's claims leak onto wrong lines).
    """
    return unicodedata.normalize(
        "NFC", s.rstrip("".join(_GREEK_PUNCT))
    ).strip().casefold()

#!/usr/bin/env python3
"""
verify_word_order.py — Verify v1.5/grk word order matches SBLGNT source.

For each verse in v1.5/grk, concatenate the colometric lines and compare
...
Usage:
    PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/verify_word_order.py
    PYTHONIOENCODING=utf-8 py -3 5-machinery/scripts/verify_word_order.py --book rom
"""
...
def load_v4_chapter(chapter_path):
    """Parse a v1.5/grk chapter file. Returns dict: verse_ref -> word list."""
...
def check_book(book_slug):
    ...
    if subdir is None:
        return [], f"v1.5/grk subdir not found for {book_slug}"

...
Reads chapter files from v4/grk/*/ (Greek) and v4/eng-kjv/*/ (English,
...
GK_PRIMARY_DIR = os.path.join(REPO_ROOT, "data", "text-files", "v4", "grk")
...
def resolve_greek_path(fpath):
    """Return the v4/grk Greek source path. Raise if missing."""
    ...
    raise FileNotFoundError(
        f"v4/grk Greek source missing for {basename} (looked under {GK_PRIMARY_DIR})"
    )
...
def build_book(prefix, chapter_files, output_dir, swap_pairs=None, quiet_set=None):
    ...
    for _ch_num, fpath in chapter_files:
        # Resolve Greek source (v4/grk preferred over v3-colometric)
...
def _run_integrity_check(book_filter=None):
    ...
    verifies that v4/grk word order matches the canonical source before
...
def main():
    ...
    if not args.skip_verify:
        print("Verifying v4/grk against SBLGNT source...")

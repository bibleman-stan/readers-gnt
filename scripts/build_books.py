...
GK_PRIMARY_DIR = os.path.join(REPO_ROOT, "data", "text-files", "v1.5", "grk")
INPUT_DIR = GK_PRIMARY_DIR  # used by discover_books for globbing all chapters (subfoldered)
EN_DIR = os.path.join(REPO_ROOT, "data", "text-files", "v1.5", "eng-kjv")
...
def resolve_greek_path(fpath):
    """Return the v1.5/grk Greek source path. Raise if missing."""
    ...
    raise FileNotFoundError(
        f"v1.5/grk Greek source missing for {basename} (looked under {GK_PRIMARY_DIR})"
    )


def resolve_english_path(fpath):
    """Return the English gloss path (KJV-verbatim, v1.5/eng-kjv/) if it exists.
...
def build_book(prefix, chapter_files, output_dir, swap_pairs=None, quiet_set=None):
    ...
    for _ch_num, fpath in chapter_files:
        # Resolve Greek source (v1.5/grk preferred over v3-colometric)
        gk_path, _gk_tag = resolve_greek_path(fpath)
        chapter_num, gk_verses = parse_chapter(gk_path)
        if chapter_num is None:
            chapter_num = _ch_num

        # Resolve English source (KJV-verbatim, v1.5/eng-kjv/)
        en_lookup = None
...
def _run_integrity_check(book_filter=None):
    ...
    verifies that v1.5/grk word order matches the canonical source before
...
def main():
    ...
    if not args.skip_verify:
        print("Verifying v1.5/grk against SBLGNT source...")

#!/usr/bin/env python3
"""
Automatic mechanical fixer for v4/grk colometric files.
...
# Map from v4/grk file prefix to Macula XML filename
PREFIX_TO_XML = {
    'matt':    '01-matthew.xml',
    'mark':    '02-mark.xml',
    'luke':    '03-luke.xml',
...
# Map from v4/grk file prefix to MorphGNT filename
PREFIX_TO_MORPHGNT = {
...
def parse_v4_file(filepath):
    """
    Parse a v4/grk file into a list of verse blocks.
...
def process_file(filepath, vocatives, chapter_num, morphgnt_data=None, dry_run=False):
    """Process a single v4/grk file. Returns list of fix records."""
...
def main():
    parser = argparse.ArgumentParser(description='Auto-fix mechanical colometric violations in v4/grk files')
    ...
    # Gather all v4/grk files from book subfolders
    files = []
    for book_dir in sorted(os.listdir(V4_DIR)):
        ...
        for fname in sorted(os.listdir(book_path)):
            if fname.endswith('.txt'):
                files.append(fname)
    print(f"Found {len(files)} v4/grk files")
    if args.dry_run:
        print("DRY RUN — no files will be modified\n")
    else:
        print("LIVE RUN — files will be modified\n")

    # Pre-load vocatives for all books that have v4/grk files
    book_vocatives = {}
    ...
    # Pre-load MorphGNT data for all books that have v4/grk files
    book_morphgnt = {}
